...
//x,y:坐标
//返回值:此点的颜色
u16 LCD_ReadPoint(uint16_t x,uint16_t y)
{
...
//  theight = sy + height - 1;
	for(h=0;h<height;h++)
	{
		for(w=0;w<width;w++)
		{
			dbuf[h*width+w]=LCD_ReadPoint(sx+w,sy+h); 
 		}
	}
}
...
{ 	  
	...
	GPIO_Initure.Pin=GPIO_PIN_5;          	//PB15,背光控制 pb15----->pf10%%%%
...
//	HAL_SRAM_Init(&TFTSRAM_Handler,&FSMC_ReadWriteTim,&FSMC_WriteTim);	
    ...
    if(lcddev.id==0X9341||lcddev.id==0X7789||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//如果是这几个IC,则设置WR时序为最快
    {
        ...
        FMC_Bank1E_R ->BWTR[6]|=3<<0;     //地址建立时间(ADDSET)为4个HCLK =24ns
        if(lcddev.id==0X7789)           //7789独立设置,否则摄像头实验可能有问题
        {
            FMC_Bank1E_R ->BWTR[6]|=3<<8; //数据保存时间(DATAST)为6ns*4个HCLK=24ns
        }
        else
        {
	...
	LCD_Display_Dir(0);		//默认为竖屏  000000000000
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, 1);			//点亮背光
	LCD_Clear(WHITE);
}  
//清屏函数
//color:要清屏的填充色
